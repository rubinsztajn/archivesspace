...
describe "ArchivesSpace user interface" do
  ...
  describe "Sub-Record Ordering" do
    before(:all) do
      login_as_archivist

      @agent_a = create_agent("John Entwistle")
      @agent_b = create_agent("Jimi Hendrix")
      @agent_c = create_agent("Keith Moon")
    end

    it "orders linked agents on an accession upon create" do
      $driver.find_element(:link, "Create").click
      $driver.find_element(:link, "Accession").click
      $driver.clear_and_send_keys([:id, "accession_title_"], "Vinyl Record Collection")
      $driver.complete_4part_id("accession_id_%d_")
      $driver.clear_and_send_keys([:id, "accession_accession_date_"], "2012-01-01")
      $driver.clear_and_send_keys([:id, "accession_content_description_"], "A box containing our own universe")
      $driver.clear_and_send_keys([:id, "accession_condition_description_"], "Slightly squashed")

      # add two agent link records
      $driver.find_element(:css => "#accession_linked_agents_ .subrecord-form-heading .btn").click
      $driver.find_element(:css => "#accession_linked_agents_ .subrecord-form-heading .btn").click

      $driver.find_element(:id => "accession_linked_agents__0__role_").select_option("creator")
      $driver.clear_and_send_keys([:id, "token-input-accession_linked_agents__0__ref_"], @agent_b)
      $driver.find_element(:css, "li.token-input-dropdown-item2").click

      $driver.find_element(:id => "accession_linked_agents__1__role_").select_option("source")
      $driver.clear_and_send_keys([:id, "token-input-accession_linked_agents__1__ref_"], @agent_a)
      $driver.find_element(:css, "li.token-input-dropdown-item2").click
      ...
      tokens = $driver.find_elements(:css => "#accession_linked_agents_ .token")
      tokens[0].text.should eq(@agent_b)
      tokens[1].text.should eq(@agent_a)
    end


    it "retains order of linked agents on an accession after drag and drop" do
      $driver.find_element(:link, "Edit").click

      $driver.find_element(:css => "#accession_linked_agents_ .subrecord-form-heading .btn").click

      $driver.find_element(:id => "accession_linked_agents__2__role_").select_option("subject")
      $driver.clear_and_send_keys([:id, "token-input-accession_linked_agents__2__ref_"], @agent_c)
      $driver.find_element(:css, "li.token-input-dropdown-item2").click

      drag_handle = $driver.find_elements(:css => "#accession_linked_agents_ .drag-handle")[2]
      $driver.action.click_and_hold(drag_handle).perform
      $driver.action.move_to($driver.find_elements(:css => "#accession_linked_agents_ .subrecord-form-wrapper")[0]).
              release.
              perform

      #source = $driver.find_elements(:css => "#accession_linked_agents_ .drag-handle")[2]
      #target = $driver.find_elements(:css => "#accession_linked_agents_ li")[0]
      #$driver.action.drag_and_drop(source, target).perform
      ...
      tokens = $driver.find_elements(:css => "#accession_linked_agents_ .token")
      tokens[0].text.should eq(@agent_c)
      tokens[1].text.should eq(@agent_b)
      tokens[2].text.should eq(@agent_a)
    end
